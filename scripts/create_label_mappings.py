...
def create_genus_label_mapping(csv_path: str) -> dict:
    ...
    idx_to_count = {
        idx: genus_counts.get(genus, 0)
        for idx, genus in idx_to_genus.items()
    }
    
    # Count species per genus
    genus_to_species_count = {
        genus: len(species_list)
...
def main():
    ...
    for idx in range(min(5, len(genus_mapping['idx_to_genus']))):
        ...
        print(f"  {idx:3d} → {genus:15s} ({count:5,} samples, {len(species_list)} species)")
        print(f"       Species: {', '.join(species_list[:5])}{'...' if len(species_list) > 5 else ''}")
    ...
    print(f"   Reduction:     {species_mapping['metadata']['num_classes'] / genus_mapping['metadata']['num_classes']:.1f}x")
