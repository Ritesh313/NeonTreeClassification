...
def test_inference(
        ...
        for idx, (i, row) in enumerate(sample_df.iterrows(), 1):
            ...
            for j, pred in enumerate(result["predictions"], 1):
                if taxonomic_level == "species":
                    ...
                    print(
                        f"   {is_correct} {j}. [{pred['probability']:6.2%}] {code:10s} - {name[:50]}"
                    )
                else:
                    genus = pred["genus"]
    ...
    if len(batch_images) > 0:
        ...
        for i, (crown_id, result) in enumerate(zip(batch_ids, batch_results), 1):
            ...
            print(
                f"   {i}. Crown {crown_id}: {label} ({result['top_probability']:.2%})"
            )
...
def main():
    ...
    parser.add_argument(
        "--hdf5_path", type=str, required=True, help="Path to neon_dataset.h5"
    )
    parser.add_argument(
        "--taxonomic_level",
        type=str,
    ...
    parser.add_argument(
        "--num_samples", type=int, default=5, help="Number of samples to test"
    )
    parser.add_argument(
        "--top_k", type=int, default=5, help="Number of top predictions to show"
    )
